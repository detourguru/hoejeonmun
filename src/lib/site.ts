...
- 캐스팅 캘린더 배우 필터가 페어(같이 나오는 회차)만 보여주던 것을, 선택한 배우 아무나 나오는 회차를 다 보여주도록 수정
- 이벤트 업로드 확인 화면에서 적용 후가 아니라 등록 전에 바로 포함/제외 회차를 조정할 수 있도록 수정
- 캐스팅 캘린더 진입 시 오늘 일정이 없으면 이번 달 첫 일정이 아니라 가장 가까운 일정으로 선택되도록 수정
- 전체 목록 화면에서 날짜 필터를 과거로 넓히면 종료된 공연도 "공연완료" 상태로 찾을 수 있도록 수정
- 캐스팅 캘린더에서 배우를 선택하지 않았을 때 캐스팅 정보가 아예 안 보이던 문제 수정`;
