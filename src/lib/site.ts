...
- 애정배우마다 나만의 별칭(이모지 등)을 정해 달력·목록에 그 이름으로 볼 수 있도록 추가
- 달력에서 날짜를 고를 때 한 주가 밀리거나 이벤트 막대가 끊겨 보이던 문제 수정
- 설치한 앱의 첫 화면에서 뒤로가기를 누르면 바로 꺼지지 않고 "한 번 더 누르면 종료돼요" 안내가 뜨도록 개선`;
